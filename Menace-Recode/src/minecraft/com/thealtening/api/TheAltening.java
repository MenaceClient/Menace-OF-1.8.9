...
public class TheAltening extends HttpUtils {
    ...
    private final String endpointV2 = "https://api.thealtening.com/v2/";
    ...
    public LicenseData getLicenseDataV2() {
        try {
            final String connectionData = connect(endpointV2 + "license?key=" + apiKey);
            return gson.fromJson(connectionData, LicenseData.class);
        } catch (IOException e) {
            if (e.getMessage().contains("401")) {
                logger.info("Invalid API Key provided");
    ...
    public AccountData getAccountDataV2() {
        try {
            final String connectionData = connect(endpointV2 + "generate?key=" + apiKey + "&info=true");
            return gson.fromJson(connectionData, AccountData.class);
        } catch (IOException e) {
            if (e.getMessage().contains("401")) {
                logger.info("Invalid API Key provided");
