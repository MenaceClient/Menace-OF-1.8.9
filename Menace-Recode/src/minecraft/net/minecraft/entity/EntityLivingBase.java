...
public abstract class EntityLivingBase extends Entity
{
    ...
    public void dismountEntity(Entity entityIn)
    {
        ...
        double y = entityIn.getEntityBoundingBox().minY + (double)entityIn.height;
            ...
            for (int k = -i; k < i; ++k)
            {
                if (j != 0 || k != 0)
                {
                    ...
                    if (this.worldObj.getCollisionBoxes(axisalignedbb).isEmpty())
                    {
                        ...
                        {
                            x = this.posX + (double)j;
                            y = this.posY + 1.0D;
                            z = this.posZ + (double)k;
                        }
                    }
                }
            }
    ...
    protected void jump()
    {
        EventJump jumpEvent = new EventJump(this.getJumpUpwardsMotion());
        jumpEvent.call();
        if(jumpEvent.isCancelled())
            return;

        this.motionY = jumpEvent.getUpwardsMotion();
