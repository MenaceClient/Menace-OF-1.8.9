...
public class Main
{
    public static void main(String[] p_main_0_)
    {
        ...
        OptionSpec<String> menaceUUID = optionparser.accepts("menaceUUID").withRequiredArg();
        OptionSpec<String> optionspec19 = optionparser.nonOptions();
        OptionSet optionset = optionparser.parse(p_main_0_);
        List<String> list = optionset.valuesOf(optionspec19);

        if (optionset.valueOf(menaceUUID) != null) {
            MenaceUUIDHandler.parseUUID(optionset.valueOf(menaceUUID));
        } else {
            throw new RuntimeException("Menace UUID is null, please re-download the client.");
        }
