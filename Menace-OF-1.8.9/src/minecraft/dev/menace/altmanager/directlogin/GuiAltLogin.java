...
extends GuiScreen {
	...
	protected void actionPerformed(GuiButton button) {
		...
		case 0: {
			switch (loginModeName) {
			case ALTENING:
				this.thread = new AltLoginAltening(this.username.getText(), this.password.getText());
				break;
			case MICROSOFT:
				this.thread = new AltLoginThreadMsa(this.username.getText(), this.password.getText());
				break;
			case MICROSOFT_BROWSER:
				this.thread = new AltLoginThreadBrowser();
				break;
			case MOJANG:
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		String status = null;
		if (thread instanceof AltLoginThread) {
			status = ((AltLoginThread)thread).getStatus();
		} else if (thread instanceof AltLoginThreadMsa) {
			status = ((AltLoginThreadMsa)thread).getStatus();
		} else if (thread instanceof AltLoginThreadBrowser) {
			status = ((AltLoginThreadBrowser)thread).getStatus();
		} else if (thread instanceof AltLoginAltening) {
			status = ((AltLoginAltening)thread).getStatus();
		}
