package dev.menace.gui.clickgui.lemon.click.component.components;
...
import dev.menace.gui.clickgui.lemon.click.component.components.sub.Checkbox;
import dev.menace.gui.clickgui.lemon.click.component.components.sub.Keybind;
import dev.menace.gui.clickgui.lemon.click.component.components.sub.ModeButton;
import dev.menace.gui.clickgui.lemon.click.component.components.sub.Slider;
import dev.menace.gui.clickgui.lemon.click.component.components.sub.VisibleButton;
		...
		if (!mod.settings.isEmpty()) {
			for (Setting set : mod.settings) {
				if(set instanceof StringSetting) {
					this.subcomponents.add(new ModeButton((StringSetting)set, this, opY));
					opY += 12;
				}
				if(set instanceof DoubleSetting) {
					Slider slider = new Slider((DoubleSetting)set, this, opY);
					this.subcomponents.add(slider);
					opY += 12;
				}
				if(set instanceof BoolSetting) {
					Checkbox check = new Checkbox((BoolSetting)set, this, opY);
