...
public class ElementCheckBox extends Element {
	 ...
	 * Konstrukor
	 */
	public ElementCheckBox(ModuleButton iparent, BoolSetting iset) {
		parent = iparent;
		set = iset;
		super.setup();
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		Gui.drawRect(x + 1, y + 2, x + 12, y + 13, ((BoolSetting)set).isChecked() ? color : 0xff000000);
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		if (mouseButton == 0 && isCheckHovered(mouseX, mouseY)) {
			((BoolSetting)set).setChecked(!((BoolSetting)set).isChecked());
			return true;
		}
