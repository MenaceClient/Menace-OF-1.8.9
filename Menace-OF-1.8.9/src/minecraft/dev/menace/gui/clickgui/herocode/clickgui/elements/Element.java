...
public class Element {
	...
	public void update(){
		...
		if(set instanceof BoolSetting){
			...
			height = comboextended ? ((StringSetting)set).getOptions().size() * (FontUtil.getFontHeight() + 2) + 15 : 15;
			
			setstrg = sname.substring(0, 1).toUpperCase() + sname.substring(1, sname.length());
			int longest = FontUtil.getStringWidth(setstrg);
			for (String s : ((StringSetting)set).getOptions()) {
		...
		}else if(set instanceof DoubleSetting){
			setstrg = sname.substring(0, 1).toUpperCase() + sname.substring(1, sname.length());
			String displayval = "" + Math.round(((DoubleSetting)set).getValue() * 100D)/ 100D;
			String displaymax = "" + Math.round(((DoubleSetting)set).getMax() * 100D)/ 100D;
