...
public class ElementSlider extends Element {
	 ...
	 * Konstrukor
	 */
	public ElementSlider(ModuleButton iparent, DoubleSetting iset) {
		parent = iparent;
		set = iset;
		dragging = false;
		super.setup();
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		String displayval = "" + Math.round(((DoubleSetting)set).getValue() * 100D)/ 100D;
		...
		double percentBar = (((DoubleSetting)set).getValue() - ((DoubleSetting)set).getMin())/(((DoubleSetting)set).getMax() - ((DoubleSetting)set).getMin());
		 ...
		 */
		if (this.dragging) {
			double diff = ((DoubleSetting)set).getMax() - ((DoubleSetting)set).getMin();
			double val = ((DoubleSetting)set).getMin() + (MathHelper.clamp_double((mouseX - x) / width, 0, 1)) * diff;
			((DoubleSetting)set).setValue(val); //Die Value im Setting updaten
		}
