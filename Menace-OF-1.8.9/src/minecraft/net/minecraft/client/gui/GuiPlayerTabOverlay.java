...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public static String getTabName(final EntityPlayer player) {
        String name = "";
        final Minecraft mc = Minecraft.getMinecraft();
        final Iterator i = (Iterator)getPlayerListTeams().iterator();
        while (((Iterator)i).hasNext()) {
            final NetworkPlayerInfo playerInfo = ((Iterator<NetworkPlayerInfo>)i).next();
            final String tabName = mc.ingameGUI.getTabList().getPlayerName(playerInfo);
            if (tabName.contains(player.getName()) && !player.getName().equals(tabName)) {
                name = tabName;
            }
        }
        return name;
    ...
    public static List<NetworkPlayerInfo> getPlayerListTeams() {
        final NetHandlerPlayClient netHandler = Minecraft.getMinecraft().getNetHandler();
        return (List<NetworkPlayerInfo>)GuiPlayerTabOverlay.field_175252_a.sortedCopy((Iterable)netHandler.getPlayerInfoMap());
    }
}
